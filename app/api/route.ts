...
export async function POST (request: Request) {
    ...
    const i: Inputs = res.inputs
    const c: Conditions = res.conditions
    const u: ConditionsUnits = res.conditionsUnits
    const m: Misc = res.misc
    const flowType: string = res.flowType
    ...
    return Response.json({ totalK, leq })
}
